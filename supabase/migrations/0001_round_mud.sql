...
-- Create function for finding nearby events
...
BEGIN
  ...
  WHERE ST_DWithin(
    location,
    ST_SetSRID(ST_MakePoint(ref_longitude, ref_latitude), 4326),
    radius_meters
  )
  ORDER BY location <-> ST_SetSRID(ST_MakePoint(ref_longitude, ref_latitude), 4326);
END;
